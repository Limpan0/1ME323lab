// Globala variabler
var myApiKey = "3c27cafdf3657c783930aded45481ef7";	// Ersätt DIN-API-KEY med din egen API key
//secret 929a82ecbdfdc73f
...
function requestLocation(id) {
	let request = new XMLHttpRequest(); // Object för Ajax-anropet
	request.open("GET","https://api.flickr.com/services/rest/?api_key=" + myApiKey + "&photo_id=" + id + "&method=flickr.photos.geo.getLocation&format=json&nojsoncallback=1",true);
	request.send(null); // Skicka begäran till servern
	request.onreadystatechange = function () { // Funktion för att avläsa status i kommunikationen
		if (request.readyState == 4)
...
function requestImgsByLocation(lat,lon) {
	let request = new XMLHttpRequest(); // Object för Ajax-anropet
	request.open("GET","https://api.flickr.com/services/rest/?api_key=" + myApiKey + "&method=flickr.photos.search&lat=" + lat + "&lon=" + lon + "&per_page=5&format=json&nojsoncallback=1&has_geo=1",true); //länken hämtar bilder från samma latitud och longitud
	request.send(null); // Skicka begäran till servern
	request.onreadystatechange = function () { // Funktion för att avläsa status i kommunikationen
		if (request.readyState == 4)
